...
class EvaluatorPolicy:
    ...
    # P2-W2: continuously scale the fidelity penalty (1.0 = full defense, 0.0 =
    # equivalent to apply_transformation_penalty=False). Lets the dose-response
    # experiment dial the invariant's strength rather than only toggle it.
    transformation_penalty_scale: float = 1.0
    ...
    # E3.1 (soft-binding / watermark mismatch detector): instead of the brittle
    # exact-hash check, compare the segment's SimHash watermark to its
    # predecessor's; a VERBATIM/EXTRACTION claim whose watermark similarity falls
    # below ``watermark_threshold`` did not preserve its input (regeneration) and
    # is rescored as GENERATION. Tolerates cosmetic edits, catches regeneration.
    detect_watermark_mismatch: bool = True
    watermark_threshold: float = 0.85
...
class WarrantEvaluator:
    ...
    def _score_warrant(self, manifest: CAPMManifest, reasons: list[str],
        ...
        prev_watermark: Optional[str] = None
        for seg in manifest.segments:
            t = TransformationType(seg.transformation)
            # E3.1: a relay claiming VERBATIM/EXTRACTION asserts it preserved its
            # input. Verify via the soft-binding WATERMARK (SimHash) when present
            # — similarity below threshold means the input's watermark did not
            # survive (regeneration) -> the claim is a lie, rescored GENERATION.
            # Falls back to the exact content-hash check if watermarks are absent.
            faithful_claim = t in (TransformationType.VERBATIM,
                                   TransformationType.STRUCTURED_EXTRACTION)
            if faithful_claim and prev_content_hash is not None:
                if (self.policy.detect_watermark_mismatch and seg.watermark
                        and prev_watermark):
                    sim = _wm.similarity(seg.watermark, prev_watermark)
                    if sim < self.policy.watermark_threshold:
                        reasons.append(
                            f"seg{seg.segment_index}: claims {t.value} but watermark "
                            f"similarity {sim:.2f} < {self.policy.watermark_threshold} "
                            f"-> soft-binding/watermark mismatch, scored as generation")
                        t = TransformationType.GENERATION
                elif (self.policy.detect_transformation_lies
            ...
            if self.policy.apply_transformation_penalty:
                # scale defaults to 1.0 (unchanged); round keeps warrant integral.
                current -= round(t.fidelity_penalty * self.policy.transformation_penalty_scale)
            if seg.crosses_org_boundary and self.policy.cross_org_aware:
                crossings += 1
                current -= per_cross
            current = max(0, current)
            prev_content_hash = seg.content_hash
            prev_watermark = seg.watermark
