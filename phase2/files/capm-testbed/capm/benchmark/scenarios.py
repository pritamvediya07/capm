...
def build_chain(*, n_hops: int = 3,
                ...
                relay_responder: Optional[Callable] = None,
                orgs: Optional[list[str]] = None) -> Scenario:
    ...
    orgs:
        Optional explicit org names, head->tail (``orgs[0]`` is the
        principal-facing agent's org, ``orgs[-1]`` is the origin/data-owner's
        org). When omitted, generic ``org-A..org-N`` names are generated. The
        cross-org benchmark (E5.4) passes real, named data-owner orgs (e.g.
        ``["org:finance-user", "org:biller"]``) so the manifest's segment orgs
        reflect genuine organisational boundaries, not anonymous placeholders.
        Must have length ``n_hops`` and contain distinct names.
    attack:
    ...
    # build tail -> head. orgs are indexed head(0)..tail(n_hops-1); the loop
    # below builds tail-first, so it indexes orgs[i] directly.
    if orgs is not None:
        if len(orgs) != n_hops:
            raise ValueError(f"orgs must have length n_hops={n_hops}, got {len(orgs)}")
        if len(set(orgs)) != len(orgs):
            raise ValueError(f"orgs must be distinct (every hop is its own org): {orgs}")
    else:
