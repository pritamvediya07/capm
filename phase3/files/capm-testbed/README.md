...
Beyond the S0–S3 ladder, the testbed now ships the full **E-series** experiment
suite from `EXPERIMENT_IMPLEMENTATION_CHECKLIST.md` (adaptive adversaries E3.x,
ablations E8.x, frontier E7.1, overhead E6.x, stats E9.3, ProVerif E2.1, and
dependency-gated stand-ins for the real-model/real-corpus experiments E4.x/E5.x).
See **`docs/TESTBED.md`** for the infrastructure map. Highlights:

| Command | Shows |
|---|---|
| `python -m experiments.e1_1_main_matrix` | headline ASR table + Wilson CIs + McNemar vs every baseline |
| `python -m experiments.e3_2_origin_capture` | the honest boundary: class-lie accepted but attributable (kills "by construction") |
| `python -m experiments.e3_3_manifest_forgery` | every forgery → REJECT (run with `CAPM_USE_SAGA=1` on real crypto) |
| `python -m experiments.e3_4_collusion` | warrant stays origin-bounded regardless of #colluders |
| `python -m experiments.e8_ablations` | each removed component raises ASR / trades soundness |
