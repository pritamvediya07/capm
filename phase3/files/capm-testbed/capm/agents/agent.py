...
# ---------------------------------------------------------------------------
# Default-responder resolution (E4.1 real-model wiring)
# ---------------------------------------------------------------------------
def _resolve_default_responder():
    """Pick the default responder for a fresh agent.

    Opt-in real-model path: if ``CAPM_AGENT_USE_LLM`` is truthy, agents emit via
    a real model (Claude or Gemini per ``CAPM_LLM_PROVIDER``) wrapped so the
    *classified actual* transformation is stamped into the manifest. Lazily
    imported to avoid an import cycle and to keep the core testbed API-free.
    Falls back to the deterministic relay when the env is unset or the backend
    is unavailable.
    """
    if not os.environ.get("CAPM_AGENT_USE_LLM"):
        return CAPMAgent._default_responder
    try:
        from capm.agents.responders import relay_responder
        return relay_responder(mode="paraphrase", classify=True)
    except Exception:
        return CAPMAgent._default_responder
...
class CAPMAgent(LocalAgent):
    ...
    def __init__(self, did: str, org: str, identity: AgentIdentity,
        ...
        # Responder resolution (E4.1): an explicit responder wins; otherwise, when
        # CAPM_AGENT_USE_LLM is set, agents emit via a REAL model call whose
        # *actual* transformation is classified and stamped into the manifest
        # (so the evaluator applies the right fidelity penalty). With the env
        # unset the safe deterministic responder is used, keeping the offline
        # test suite fully reproducible and API-free.
        self.responder = responder or _resolve_default_responder()
    ...
    def _emit(self, value: WarrantedValue, to_org: str,
        ...
        # perceptual watermark (SimHash) for relay-fidelity verification (E3.1):
        # survives cosmetic edits, destroyed by regeneration.
        from capm.manifest import watermark as _wm
        mark = _wm.fingerprint(value.content)
        seg = manifest.append_segment(
            ...
            soft_binding=soft, watermark=mark, timestamp=self._clock())
