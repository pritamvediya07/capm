...
class CredentialRegistry:
    ...
    def trusts(self, did: str) -> bool:
        return did in self._by_did and did not in self._revoked

    def revoke(self, did: str) -> bool:
        """Post-hoc credential revocation (the trust root's response to a captured
        origin, E3.2). After revocation the DID no longer ``trusts``, so any
        manifest whose chain includes a segment signed by it fails verification
        and is REJECTed. Returns True if the DID was known.

        This is the composable layer that closes the origin-capture residual:
        warrant alone cannot detect a class lie, but the manifest still
        *attributes* the bad claim to this signing identity, and revoking it here
        stops every subsequent (and replayed) claim from the captured origin."""
        known = did in self._by_did
